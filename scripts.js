  ...
  video.addEventListener("play", async () => {
    ...
    setInterval(async () => {
      ...
      resizedDetections.forEach((face) => {
        const { gender, genderProbability, expressions } = face;
        const gendetText = `${gender} - ${Math.floor(
          genderProbability * 100
        )}%`;
        // const ageText = `Age: ${Math.round(age)} years`;
        const expressionText = `Expressions: ${Object.keys(expressions)
          .map((exp) => `${exp}: ${expressions[exp].toFixed(2)}`)
          .join(", ")}`;
        const textfield = new faceapi.draw.DrawTextField(
          [gendetText],
          face.detection.box.topLeft
        );
